...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # construct linear projection layer for concatenating image CLIP embedding, RT and prompt CLIP embedding
        self.ccc_projection = nn.Linear(1284, 768, dtype=torch.float32)
        nn.init.eye_(list(self.ccc_projection.parameters())[0][:768, :768])
        nn.init.zeros_(list(self.ccc_projection.parameters())[1])
        self.ccc_projection.requires_grad_(True)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        with torch.enable_grad():
            ...
            cond["c_crossattn"] = [
                self.cc_projection(torch.cat([torch.where(prompt_mask, null_prompt, clip_emb), T[:, None, :]], dim=-1))]

...
def load_objaverse_model(ckpt_root):
    ckpt_fname = '../zero123/checkpoint/105000.ckpt'
...
class StableDiffusion(ScoreAdapter):
    ...
    def denoise_objaverse(self, xs, σ, score_cond, grad=False):
        if grad:
            with self.precision_scope("cuda"):
                N = xs.shape[0]
                cond_t, σ = self.time_cond_vec(N, σ)
    ...
    @torch.no_grad()
    def prompts_emb_2(self, prompts):
        assert isinstance(prompts, list)
        with self.precision_scope("cuda"):
            inputs = self.text_tokenizer(prompts, padding=True, return_tensors="pt")
            outputs = self.text_model(**inputs)
            cond = outputs.text_embeds.cuda()
            return cond
        ...
        if conditioning_key == 'hybrid':
            ...
            with self.precision_scope("cuda"):
                with self.model.ema_scope():
                    ...
                    cond['c_concat'] = [
                        torch.cat([torch.zeros_like(self.vae_emb).to(self.device), self.vae_emb], dim=0)]
    ...
    def img_prompt_emb(self, T=None):
        assert T is not None, 'for objaverse, needs to input T (viewpoint vector)'
        with self.precision_scope("cuda"):
            cond = {}
            clip_emb = self.model.module.ccc_projection(
                torch.cat([self.clip_emb, T[None, None, :], self.prompt_emb[None, :]], dim=-1))

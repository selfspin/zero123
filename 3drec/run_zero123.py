...
def sjc_3d(poser, vox, model: ScoreAdapter,
    ...
    image_mask = cv2.resize(image_mask.astype(np.float32), dsize=(256, 256), interpolation=cv2.INTER_NEAREST).astype(
        bool)
    background_mask = cv2.resize(background_mask.astype(np.float32), dsize=(H, W),
    ...
    with tqdm(total=n_steps) as pbar, \
        ...
        for i in range(n_steps):
            ...
            T = get_T(T_target[None, :], T_cond[None, :]).to(model.device)
